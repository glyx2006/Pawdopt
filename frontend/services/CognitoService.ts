...
interface StructuredAddress {
  formatted: string;
  // You can add other address components here if Cognito provides them
...
interface CustomJwtPayload extends JwtPayload {
  ...
  name: string;
  phone_number: string;
  address: string; // Address claim from JWT is often a stringified JSON
...
export const getCurrentUserAttributes = async (): Promise<UserAttributes | null> => {
    ...
    try {
        ...
        // Initialize parsedAddress with a default empty structure
        let parsedAddress: StructuredAddress = { formatted: '' }; 

        // Attempt to parse the address if it's a string from the JWT
        if (typeof decodedToken.address === 'string') {
            try {
                const tempAddress = JSON.parse(decodedToken.address);
                if (typeof tempAddress.formatted === 'string') {
                    parsedAddress = tempAddress;
                    console.log("Successfully parsed address from JSON.");
                } else {
                    parsedAddress = { formatted: decodedToken.address }; 
                    console.log("Parsed JSON had no 'formatted' key, using raw address string.");
                }
            } catch (e) {
                console.warn("Could not parse address string as JSON:", decodedToken.address, e);
                parsedAddress = { formatted: decodedToken.address }; 
            }
        } else if (typeof decodedToken.address === 'object' && decodedToken.address !== null) {
            parsedAddress = decodedToken.address as StructuredAddress;
            console.log("Address was already a structured object.");
        }

        const attributes: UserAttributes = { // Explicitly type the attributes object
            'sub': decodedToken.sub || '',
            'email': decodedToken.email || '',
            'name': decodedToken.name || '',
            'phone_number': decodedToken.phone_number || '',
            'address': parsedAddress, // Assign the correctly typed object here
